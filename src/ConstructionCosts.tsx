...
function calculateFairScore(prices) {
  // Beispielhafte Gewichtung – je niedriger die Preise, desto höher der Score
  const maxReference = [500, 1200, 200, 10000, 8]; // Annahme: Referenz für hohe Preise
  const total = prices.reduce((acc, item, i) => {
    const ratio = item.price / maxReference[i]; // z. B. 420/500 = 0.84
    return acc + ratio;
  }, 0);
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <main className="flex flex-col px-6 pt-8 pb-24">
        ...
        <div className="bg-purple-50 p-6 rounded-xl shadow-md space-y-4 mb-10">
          <h2 className="text-xl font-semibold text-[#573A6F] mb-2">Rohstoffpreise (live)</h2>
          <ul className="space-y-2 text-lg text-[#573A6F]">
        ...
        <div className="bg-[#573A6F] text-white rounded-xl p-6 text-center shadow-lg">
          <h3 className="text-2xl font-bold mb-2">Fair-Score: {score} / 100</h3>
          <p className="text-lg">{scoreText}</p>
        </div>
      </main>
